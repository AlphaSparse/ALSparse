...
cuda_mm()
{
  ...
  std::cout << "cusparseSpMM_bufferSize time: " << elapsed_time << " microseconds" << std::endl;
  void* dBuffer = NULL;
  ...
  GPU_TIMER_END(elapsed_time, event_start, event_stop);
  std::cout << "cusparseSpMM_compute time: " << elapsed_time/trials<< " microseconds" << std::endl;
  // Clear up on device
  cudaFree(dArow);
  cudaFree(dAcol);
  cudaFree(dAval);
...
alpha_mm()
{
  ...
  alphasparseSpMM_bufferSize(handle,
                             ...
                             &bufferSize);
  GPU_TIMER_END(elapsed_time, event_start, event_stop);
  std::cout << "alphasparseSpMM_bufferSize time: " << elapsed_time << " microseconds" << std::endl;
  void* dBuffer = NULL;
  CHECK_CUDA(cudaMalloc(&dBuffer, bufferSize));
  ...
  GPU_TIMER_END(elapsed_time, event_start, event_stop);
  std::cout << "alphasparseSpMM_compute time: " << elapsed_time/trials << " microseconds" << std::endl;
  // Clear up on device
  cudaFree(dArow);
  cudaFree(dAcol);
  cudaFree(dAval);
