...
cuda_mm()
{
  ...
  GPU_TIMER_END(elapsed_time, event_start, event_stop);
  std::cout << "cusparseSpGEMM_compute2 time: " << elapsed_time << " microseconds" << std::endl;
...
alpha_mm()
{
  ...
  GPU_TIMER_END(elapsed_time, event_start, event_stop);
  std::cout << "alphasparseSpGEMM_compute2 time: " << elapsed_time << " microseconds" << std::endl;
  // allocate matrix C
  nnz_c = matC->nnz;
  ...
  // CHECK_CUDA(cudaMemcpy(
  //   matC_ptr, matC->row_data, matC->rows * sizeof(int), cudaMemcpyDeviceToHost))
